...
import { Pencil, Trash2, User, Building2, DollarSign, Percent, Calendar, TrendingUp, FileText } from "lucide-react";
    ...
    if (calculationType === "convertible" && "interest_rate" in instrument) {
      ...
      if (
        ...
        details.push({
          ...
          value: new Date(instrument.expected_conversion_date).toLocaleDateString(),
          icon: <Calendar className="h-3.5 w-3.5" />
        });
      }
      if ("discount_rate" in instrument) {
        details.push({
          label: "Discount",
          value: `${decimalToPercentage(instrument.discount_rate).toFixed(2)}%`,
