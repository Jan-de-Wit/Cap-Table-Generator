...
import { CheckCircle2, Pencil, Percent, Building2, User, Shield, ArrowUpDown, ChevronDown } from "lucide-react";
...
export function ProRataExerciseSection({
  ...
  const getProRataData = (holderName: string, defaultType: "standard" | "super") => {
    ...
    if (existingProRata && "pro_rata_type" in existingProRata) {
      return {
        holder_name: "holder_name" in existingProRata ? existingProRata.holder_name : holderName,
        class_name: "class_name" in existingProRata ? existingProRata.class_name : "",
        pro_rata_type: existingProRata.pro_rata_type,
        pro_rata_percentage: "pro_rata_percentage" in existingProRata 
    ...
    const rightsData = holdersWithProRataRights.find(r => r.holderName === holderName);
    return {
  ...
  const tableData: ProRataRow[] = React.useMemo(() => {
    return holdersWithProRataRights.map(({ holderName, type, class_name, percentage }) => {
  ...
  const columns = React.useMemo<ColumnDef<ProRataRow>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.original.class_name}</span>
      ),
    },
    {
