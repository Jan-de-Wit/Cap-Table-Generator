...
def create_accrued_interest_formula(principal_ref: str, interest_rate_ref: str,
    ...
    if interest_type_ref:
        ...
        formula = (
            f"=IFERROR("
            f"IF({interest_type_ref}=\"no_interest\", 0, "
            f"IF({interest_type_ref}=\"simple\", {simple_formula}, "
            f"IF({interest_type_ref}=\"compound_yearly\", {compound_yearly_formula}, "
            f"IF({interest_type_ref}=\"compound_monthly\", {compound_monthly_formula}, "
            f"IF({interest_type_ref}=\"compound_daily\", {compound_daily_formula}, "
            f"{simple_formula}))))), 0)"
        )
        return formula
